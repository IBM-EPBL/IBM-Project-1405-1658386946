...
app.config['MAIL_USERNAME'] = 'cad.crregistry@gmail.com'
app.config['MAIL_PASSWORD'] = 'xofnkqrvnuluwdfo'
...
def register():
        if request.method=='POST':
            ...
            if userexist:
                ...
                msg = Message('CAD-CustomerCareRegistry Account Created Successfully',sender ='cad.crregistry@gmail.com',recipients = [email])
...
def raiseticket():
        if request.method=='POST':
                ...
                msg = Message('CAD-CustomerCareRegistry, You have raised a ticket',sender ='cad.crregistry@gmail.com',recipients = [emailid,'ktviknesh@gmail.com'])
                msg.body = ("Raised ticket details are:"+"\n\n\n"+"Ticket ID: "+ticketid+"\n\n"+"Username: "+username+"\n\n"+"Email ID: "+emailid+"\n\n"+"Key Complaint: "+kequery+"\n\n"+"Detailed Complaint: "+dequery+"\n\n"+"Admin Reply: "+adminreply+"\n\n\n\n"+"With regard,"+"\n"+"CAD-CustomerCareRegistry")
                mail.send(msg)
                return render_template('checkstatus.html')
...
def adminreply():
        if request.method=='POST':
                ...
                return render_template('adminreply.html',ticket_id=ticketid,user_name=username,email_id=emailid,key_complaint=kequery,detailed_complaint=dequery,admin_reply=adminreply)
...
def status():
        if request.method=='POST':
                ...
                while ibm_db.fetch_row(stmt) != False:
                    ...
                    dequery=ibm_db.result(stmt, "DEQUERY")
                    adminreply=ibm_db.result(stmt, "ADMINREPLY")                
        return render_template('status.html',ticket_id=ticketid,user_name=username,email_id=emailid,key_complaint=kequery,detailed_complaint=dequery,admin_reply=adminreply)
...
def ticketreply():
        if request.method=='POST':
                ...
                prep_stmt1=ibm_db.prepare(conn,insert_sql)
                ibm_db.bind_param(prep_stmt1,1,reply)
                ibm_db.bind_param(prep_stmt1,2,ticketid)
                ibm_db.execute(prep_stmt1)
                msg = Message('CAD-CustomerCareRegistry, Ticket Resolved',sender ='cad.customercareregistry@gmail.com',recipients = [emailid])
                msg.body = ("Raised ticket details are:"+"\n\n\n"+"Ticket ID: "+ticketid+"\n\n"+"Username: "+username+"\n\n"+"Email ID: "+emailid+"\n\n"+"Key Complaint: "+kequery+"\n\n"+"Detailed Complaint: "+dequery+"\n\n"+"Admin Reply: "+adminreply+"\n\n\n\n"+"With regard,"+"\n"+"CAD-CustomerCareRegistry")
                mail.send(msg)
                return render_template('admindashboard.html')         
